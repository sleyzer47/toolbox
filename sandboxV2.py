...
class MapPage(ctk.CTkFrame):
    ...
    def visualize_network(self, devices):
        ...
        for ip, info in devices.items():
            G.add_node(ip, label=f"{info.get('hostname', 'Unknown')} ({ip})")
            G.add_edge(ip, self.network_range, weight=0.1)

        pos = nx.spring_layout(G)
        nx.draw(G, pos, with_labels=True, node_color='skyblue', edge_color='red', node_size=500, font_size=12)
        plt.show()
