from pages.nmap import run_nmap_scan
...
class HostPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        button1 = ctk.CTkButton(button_frame, text="Host", fg_color="#0d68a1")
        ...
        title = ctk.CTkLabel(canvas, text="Host Page", text_color="Black", font=(None, 20))
        title.pack(side="top", pady=10, anchor="n")

        label = ctk.CTkLabel(canvas, text="Welcome in host page!", text_color="Black", font=(None, 14))
        ...
        generate_button = ctk.CTkButton(canvas, text="Generate Report", command=self.generate_report)
    ...
    def generate_report(self):
        ...
        ip = self.entry.get()
        nmap_option = ["-sS", "-A", "-sV"][self.nmap_option.get() - 1]  # Sélection de l'option Nmap en fonction du radiobutton choisi

        try:
            ipaddress.ip_address(ip)
        except ValueError:
            self.show_error_message("IP incorrect/introuvable!", self.canvas)
            return

        self.is_request_pending = True

        run_nmap_scan(ip, nmap_option)  # Appeler la fonction run_nmap_scan de la page nmap.py
